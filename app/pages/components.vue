...
import { AmbientGrid } from '~ui/ambient-grid'
import { BackgroundBeams } from '~ui/background-beams'
import { DottedGlowBackground } from '~ui/dotted-glow-background'
import { GravityStars } from '~ui/gravity-stars'
import { BackgroundRippleEffect } from '~ui/background-ripple-effect'
import { SnowEffect } from '~ui/snow-effect'
  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div v-for="group in groups" :key="group.title" class="mb-24">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        <div v-for="(item, index) in group.items" :key="item.to" class="">
          <ClientOnly>
            <Motion
              ...
              <NuxtLink :to="item.to"
                ...
                <div
                  ...
                  <ClientOnly v-if="backgroundComponents[item.label]">
                    <component :is="backgroundComponents[item.label]" class="absolute inset-0 z-0 opacity-80"
                      v-bind="item.label === 'Gravity Stars' ? { starsCount: 40 } : {}" />
                  </ClientOnly>

                  <WireframeThumbnail v-else :component="item.label"
                    class="w-full h-full group-hover:scale-105 transition-transform duration-700" />

                  <div
                    class="absolute inset-0 bg-primary/5 opacity-0 group-hover:opacity-100 transition-opacity z-10" />
                </div>
