...
import { inject, ref, watch, nextTick, onMounted, onBeforeUnmount, type ComputedRef, type Ref } from 'vue'
...
const updateIndicator = async () => {
  ...
  // Safely unwrap DOM element from Reka UI Primitive
  const el = navRef.value?.$el || navRef.value
  if (!props.indicator || !el || !(el instanceof HTMLElement)) return

  await nextTick()

  const currentId = context.activeId.value
  // Query by ID value instead of active state to avoid race conditions
...
onMounted(() => {
  if (props.indicator && typeof window !== 'undefined') {
