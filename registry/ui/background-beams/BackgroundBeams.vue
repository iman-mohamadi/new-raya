...
const props = withDefaults(defineProps<Props>(), {
  ...
  colorMid: 'var(--secondary)',
  colorTo: 'var(--accent)',
  duration: 10,
  delay: 0
})

// Extracted outside the component setup to reduce memory overhead
const paths = [
...
// Initialize with static default values to ensure SSR hydration matches the client
const animatedPaths = ref(
  paths.map((path) => ({
    d: path,
    duration: props.duration,
    delay: props.delay,
    y2End: 97 // Static fallback
  }))
)

onMounted(() => {
  // Apply the randomness only on the client after hydration
  animatedPaths.value = paths.map((path) => ({
    d: path,
    duration: Math.random() * 10 + props.duration,
    delay: Math.random() * 10 + props.delay,
...
<template>
  <div
    :class="cn(
      'absolute inset-0 z-0 flex h-full w-full items-center justify-center [mask-repeat:no-repeat] [mask-size:40px]',
      props.class
    )"
  >
    <svg
    ...
    >
      ...
      <!-- Animated beam paths -->
      <path
        v-for="(path, index) in animatedPaths"
        :key="`path-${index}`"
        :d="path.d"
      ...
      <defs>
        ...
        >
          <!-- Using standard Tailwind theme variables for dark/light mode compatibility -->
          <stop offset="0.0666667" stop-color="var(--border)" />
          <stop offset="0.243243" stop-color="var(--border)" />
          <stop offset="0.43594" stop-color="transparent" stop-opacity="0" />
        </radialGradient>
      </defs>
    </svg>
  </div>
