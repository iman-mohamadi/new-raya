...
interface GlowingEffectProps {
  ...
  spread?: number
  /**
   * default: Uses your shadcn theme colors (primary, secondary, accent)
   * mono: Uses the foreground color (adapts to light/dark mode)
   * rainbow: Uses the hardcoded holographic hex colors
   */
  variant?: 'default' | 'mono' | 'rainbow'
  glow?: boolean
  disabled?: boolean
...
const handleMove = (e?: MouseEvent | { x: number; y: number }) => {
  ...
  // Debounce with RAF to prevent excessive DOM layout thrashing
  if (animationFrameId) cancelAnimationFrame(animationFrameId)

  animationFrameId = requestAnimationFrame(() => {
    // Resolve the actual DOM element from the Motion component ref
    ...
    if (isClose) {
      ...
      // Calculate shortest path for smooth rotation to avoid 360-degree snapback
...
const gradientStyle = computed(() => {
  ...
  if (props.variant === 'mono') {
    return `repeating-conic-gradient(
      ...
      var(--foreground),
      var(--foreground) calc(25% / ${repeat})
    )`
  }

  if (props.variant === 'rainbow') {
    return `radial-gradient(circle, #dd7bbb 10%, transparent 20%),
  ...
  // Default: Theme compliant colors using shadcn variables
  return `radial-gradient(circle, var(--primary) 10%, transparent 20%),
    radial-gradient(circle at 40% 40%, var(--secondary) 5%, transparent 15%),
    radial-gradient(circle at 60% 60%, var(--accent) 10%, transparent 20%),
    repeating-conic-gradient(
      from 236.84deg at 50% 50%,
      var(--primary) 0%,
      var(--secondary) calc(33% / ${repeat}),
      var(--accent) calc(66% / ${repeat}),
      var(--primary) calc(100% / ${repeat})
    )`
})
</script>
...
<template>
  <div
    :class="cn(
      ...
      props.variant === 'mono' && 'border-foreground',
