...
const props = withDefaults(defineProps<{
  class?: string
  /** The base HSL hue value for the button (0-360) */
  hue?: number
  /** Main button text */
  label?: string
  /** Smaller subtitle text below the main text */
  subLabel?: string
  /** Whether to show the floating ambient particles */
  showParticles?: boolean
  /** Overall scale multiplier for the button */
  scale?: number
}>(), {
  hue: 350,
  label: 'START',
...
const styleVars = computed<Record<string, string | number>>(() => ({
  '--theme-hue': props.hue,
...
<template>
  <div class="pixel-button-wrapper" :style="styleVars">
    ...
    <div v-if="showParticles" class="pointer-events-none absolute -inset-12 z-[1]">
    ...
    >
      <div class="bg-effect-1 absolute inset-0 z-[1]"></div>
      <div class="bg-effect-2 absolute inset-0 z-[1]"></div>
      <div class="scanlines absolute inset-0 z-[2] pointer-events-none"></div>

      <div class="relative z-10 flex h-full w-full items-center justify-between px-5 [transform:translateZ(20px)]">
        
        <div class="flex flex-col items-center gap-1">
          <slot name="icon-left">
            <svg class="main-icon size-5 transition-all duration-300 drop-shadow-[2px_2px_0_rgba(0,0,0,0.5)]" viewBox="0 0 24 24" fill="currentColor">
            ...
            <svg class="accent-icon size-3 opacity-70" viewBox="0 0 24 24" fill="currentColor">
              <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"></path>
            </svg>
          </slot>
        </div>

        <span class="flex flex-col items-center font-black leading-none drop-shadow-[2px_2px_0_#000,4px_4px_0_rgba(0,0,0,0.3)]">
          <span class="text-[18px] tracking-[3px]">
            <slot>{{ label }}</slot>
          </span>
          <span class="mt-0.5 text-[12px] opacity-90 tracking-[2px]">
        ...
        <div class="flex flex-col items-center gap-1">
          <slot name="icon-right">
            <svg class="play-icon size-4 transition-all duration-300 drop-shadow-[2px_2px_0_rgba(0,0,0,0.5)]" viewBox="0 0 24 24" fill="currentColor">
      ...
      <div class="corner -top-1 -left-1"></div>
      <div class="corner -top-1 -right-1"></div>
      <div class="corner -bottom-1 -left-1"></div>
      <div class="corner -bottom-1 -right-1"></div>
...
.pixel-game-button:hover span.text-[18px] { animation: text-glow 500ms ease-in-out; }
