...
const partitionBarVariants = cva("flex flex-row w-full", {
  variants: {
    size: {
      sm: "text-xs",
      md: "text-sm",
...
interface Props {
  /** Gap between segments in Tailwind spacing units (e.g., 1 = 4px) */
  gap?: number
...
<template>
  <ul
    :class="cn(partitionBarVariants({ size }), props.class)"
    :style="{ gap: `${gap * 4}px` }"
  >
    <slot />
  </ul>
