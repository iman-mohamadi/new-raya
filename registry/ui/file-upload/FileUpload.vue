...
onBeforeUnmount(() => {
  ...
  files.value.forEach(file => {
    ...
    if (urlCache.has(rawFile)) {
      window.URL.revokeObjectURL(urlCache.get(rawFile)!)
    }
  })
})
...
const context = {
  ...
  removeFile: (file: File) => {
    const rawFile = toRaw(file)
    if (typeof window !== 'undefined' && urlCache.has(rawFile)) {
      window.URL.revokeObjectURL(urlCache.get(rawFile)!)
      urlCache.delete(rawFile)
    }
    files.value = files.value.filter(f => toRaw(f) !== rawFile)
      ...
      if (props.onUpload) {
        props.onUpload(accepted, {
          onProgress: (file, progress) => {
            const state = fileStates.get(toRaw(file))
            if (state) {
