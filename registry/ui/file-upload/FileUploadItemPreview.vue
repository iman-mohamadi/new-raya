...
import { FileIcon, FileTextIcon, FileCodeIcon, FileArchiveIcon, FileVideoIcon, FileAudioIcon } from 'lucide-vue-next'
...
const imageUrl = computed(() => {
  if (!isImage.value || typeof window === 'undefined') return null
  const rawFile = toRaw(file)
  if (!rootContext.urlCache.has(rawFile)) {
    rootContext.urlCache.set(rawFile, window.URL.createObjectURL(rawFile))
  }
...
<template>
  ...
  >
    <img v-if="isImage && imageUrl" :src="imageUrl" :alt="file.name" class="size-full object-cover" />
