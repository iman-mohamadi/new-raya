...
const updateScrollDuringDrag = (e: MouseEvent | TouchEvent) => {
  const currentY = (typeof window !== 'undefined' && window.TouchEvent && e instanceof TouchEvent ? e.touches?.[0]?.clientY : (e as MouseEvent).clientY) || 0;
...
const initiateDragGesture = (event: MouseEvent | TouchEvent) => {
  ...
  const startY = (typeof window !== 'undefined' && window.TouchEvent && event instanceof TouchEvent ? event.touches?.[0]?.clientY : (event as MouseEvent).clientY) || 0;
